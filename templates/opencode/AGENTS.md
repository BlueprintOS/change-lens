...
- `/change-lens-locate find|where|refs|impact|draft-concept`: read-only code location; `impact` is a static estimate and must include confidence and blind spots.
- `/change-lens-guard plan|plan --ephemeral|lock|audit|explain-escape`: scoped production-change guardrails.
...
3. Do not modify production code after locate; lock scope with `change-lens-guard` first, or use `plan --ephemeral` only for low-risk small edits.
...
6. Audit output must separate `file_scope`, `symbol_scope`, and `checks`; audit-lite is not a locked manifest audit.
